...
// let printinclipboard=``
// let arr1=["mohammad","abd","asad","ahmad","tamer"];

// let arr2= arr1.map(function(elemant,index,arr){
   
    
//    printinclipboard=` ${arr}   `
 
    

//     return  document.body.innerHTML=printinclipboard
    
// })
// Excersice 3: Write a JavaScript program to converts a comma-separated values (CSV) string to a 2D array. 
let string =`hello,world,30
hello1,world1,40`
...
let stringobject =`say,name,age
hello1,orange1,40
hello2,orange2,50`
let arrrayobject=[]
let row1=stringobject.split("\n")


let header =row1[0].split(",")
 let obj={}
for (let i = 1; i < row1.length; i++) {
   let currentrow = row1[i].split(",")

  for (let j = 0; j < header.length; j++) {
    
    obj[header[j]]= currentrow[j]
  }
  arrrayobject.push(obj)
}
console.log(obj);
console.log(arrrayobject);
...
let arrcomma=[{name:"mohammad",lname:"abd",age:22},
{name:"mohammad",lname:"abd",age:22},
{name:"mohammad",lname:"abd",age:22}

]
let arrsub=["name","lname","age"]
let joinarr=""
joinarr +=arrsub.join(",") +"\n"

for (const key of arrcomma) {

  let rowarr=[]

  for (const arrs of arrsub) {
    
    rowarr.push(key[arrs]);
  }
  joinarr +=rowarr.join(",") +"\n"
}
console.log(joinarr);
